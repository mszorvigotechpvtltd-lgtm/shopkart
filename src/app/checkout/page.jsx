  ...
  const makePurchase = async () => {
    ...
    try {
      ...
      if (
        ...
        if (!isValidNameOrLastname(f.cardName))
          return toast.error("Invalid card name");
        if (!isValidCardNumber(f.cardNumber))
          return toast.error("Invalid card number");
        if (!isValidCreditCardExpirationDate(f.expirationDate))
          return toast.error("Invalid expiration date");
        if (!isValidCreditCardCVVOrCVC(f.cvc))
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          <div className="lg:col-span-2 space-y-12">
            ...
            <div className="space-y-6">
              ...
              {"cardName cardNumber expirationDate cvc"
                .split(" ")
                .map((field, index) =>
                  renderField(
                    field,
          ...
          <div className="h-fit lg:sticky top-10  text-[#35530e]">
            <OrderSummary
              total={total}
              products={products}
              mode={"checkout"}
